...
# 🇮🇳 Isolated Tamil Mode (OCR & RAG)

PersonaForge includes a dedicated, completely isolated **Tamil Mode** for Tamil OCR, document processing, multilingual vector storage, and native Tamil response generation.
...
```
        ...
        ENGLISH (Default)                    TAMIL
               │                               │
     Existing English Pipeline          Tamil Pipeline (app/tamil/)
  ┌────────────────────────────┐    ┌─────────────────────────────────┐
  │ • PyMuPDF / Text Extract   │    │ • Detect Text vs Scanned PDF    │
  │ • English Cleaning/Split   │    │ • Tamil Preprocessor (Grayscale)│
  │ • BAAI/bge-base-en-v1.5    │    │ • PaddleOCR PP-OCRv5 (lang='ta')│
  │ • persona_forge_docs (Chr) │    │ • Tamil Cleaner (Unicode/Punct) │
  │ • English Query Planner    │    │ • Tamil Chunker (Para/Sentence) │
  │ • English Context Prompt   │    │ • Multilingual Embedding Model  │
  └────────────────────────────┘    │ • persona_forge_docs_ta (Chroma)│
                                    │ • Direct Tamil Vector Retriever │
                                    │ • Tamil Prompt & Generation     │
                                    └─────────────────────────────────┘
```

### Features:
1. **PaddleOCR PP-OCRv5 Tamil**: Automatic scanned PDF vs digital PDF detection. Scanned pages are rendered at 300 DPI, preprocessed, and recognized using PaddleOCR with Tamil (`lang='ta'`) preserving bounding boxes and confidence scores.
2. **Tamil Text Cleaner & Chunker**: Preserves Tamil Unicode characters (`\u0B80-\u0BFF`), paragraph structures, and sentence boundaries.
3. **Configurable Multilingual Embeddings**: Defaults to `sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2` (configurable via `TAMIL_EMBEDDING_MODEL`).
4. **Separate Vector Collection**: Uses `persona_forge_docs_ta`, guaranteeing 100% isolation from English documents.
5. **Direct Tamil Retrieval & Generation**: Tamil queries search directly in the Tamil collection without translation, generating authentic Tamil responses.

### Setup PaddleOCR for Tamil:
```bash
