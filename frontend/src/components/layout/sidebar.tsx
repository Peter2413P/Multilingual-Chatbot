  ...
  return (
    <div className="flex flex-col w-64 bg-zinc-950 border-r border-zinc-800 text-zinc-100 h-screen overflow-y-auto">
      ...
      <div className="p-4 border-b border-zinc-800/50">
        <div className="text-xs font-medium text-zinc-500 uppercase tracking-wider mb-2 flex items-center justify-between">
          <span className="flex items-center gap-1.5"><Globe className="w-3.5 h-3.5 text-zinc-400" /> Language Mode</span>
          <span className={cn("text-[10px] px-1.5 py-0.5 rounded font-bold uppercase", language === "ta" ? "bg-amber-500/20 text-amber-400 border border-amber-500/30" : "bg-emerald-500/20 text-emerald-400 border border-emerald-500/30")}>
            {language === "ta" ? "தமிழ்" : "English"}
          </span>
        </div>
        <div className="grid grid-cols-2 gap-1 bg-zinc-900/90 p-1 rounded-lg border border-zinc-800">
          <button
            onClick={() => setLanguage("en")}
            className={cn(
              "py-1.5 text-xs font-medium rounded transition-all text-center",
              language === "en" 
          ...
          <button
            ...
            className={cn(
              "py-1.5 text-xs font-medium rounded transition-all text-center flex items-center justify-center gap-1",
              language === "ta" 
                ? "bg-amber-600/30 text-amber-300 font-semibold border border-amber-500/50 shadow-sm" 
                : "text-zinc-400 hover:text-zinc-200"
            )}
          >
            தமிழ்
