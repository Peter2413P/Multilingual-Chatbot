...
def delete_persona(persona_id: str):
    ...
    # Also clean up isolated Tamil Chroma collection if exists
    try:
...
@router.post("/upload")
async def upload_document(persona_id: str, file: UploadFile = File(...), language: Optional[str] = "en"):
    db = SessionLocal()
    persona = db.query(Persona).filter(Persona.id == persona_id).first()
    db.close()
    if not persona:
    ...
    try:
        ...
        selected_lang = (language or "en").lower().strip()
        if selected_lang in ["ta", "tamil"]:
            from app.tamil.tamil_service import ingest_tamil_document
            doc_id = ingest_tamil_document(file_path, file.filename, persona_id)
        else:
        ...
        return {"status": "success", "id": doc_id, "filename": file.filename, "language": selected_lang}
...
async def chat_stream(request: ChatRequest):
    try:
        ...
        selected_lang = (request.language or "en").lower().strip()
        
        if selected_lang in ["ta", "tamil"]:
            from app.tamil.tamil_service import stream_tamil_chat_response
            return StreamingResponse(
                stream_tamil_chat_response(request.persona_id, request.message, history_dicts),
...
@router.get("/debug-chroma-ta/{persona_id}")
def debug_chroma_ta(persona_id: str):
    from app.tamil.tamil_vector_store import get_tamil_vector_store
    vs = get_tamil_vector_store()
    collection = vs._collection
    ...
    return {
        "total_tamil_chunks": len(metadatas),
        "metadatas": metadatas[:5] if metadatas else [],
        "first_doc": documents[0] if documents else None
    }
...
async def remove_document(doc_id: str):
    try:
        ...
        from app.tamil.tamil_service import delete_tamil_knowledge_source
        delete_tamil_knowledge_source(doc_id)
